...
首先，如何在工程中添加 ~app.manifest~ 就花了我不少工夫：

1. 右键工程属性，在 ~Win32 Resources~ 中会看到 ~Manifest~ 。
2. 这里默认选项是 ~Embed manifest with default settings~ ，必须要改为 ~Use a custom manifest~ 。
3. 然后就会看到多出一个选项 ~Manifest path~ ，此时指定准备好的 ~app.manifest~ 即可。
...
重新编译后就可以看到 ~fake_fastgithub.exe~ 图标上多了一个小盾牌，打完收工。
